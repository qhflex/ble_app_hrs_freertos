...
void qma6110p_task(void *pvParameters) {
  ...
  for (int i = 0; i < 64; i++) {
    uint8_t buf = 0xee;
    nrf_twi_mngr_transfer_t xfers[] = {QMA6110P_READ(i, &buf, 1)};
    ...
    err_code = nrf_twi_mngr_perform(&m_nrf_twi_mngr, &config, xfers,
                                    sizeof(xfers) / sizeof(xfers[0]), NULL);
    APP_ERROR_CHECK(err_code);
...
void app_qma6110p_freertos_init(void) {
  BaseType_t xReturned =
      xTaskCreate(qma6110p_task, "qma6110p", TSK_QMA6110P_STACK_SIZE, NULL,
                  TSK_QMA6110P_PRIORITY, &m_qma6110p_thread);
  if (xReturned != pdPASS) {
